...
export default function DownloadLinkBox(props) {
  ...
  const handleBack = () => {
    ...
    document.title = "NepalShares | नेपाली बाट नेपाली कै लागि";
    ...
    <>
      <Paper className={classes.container}>
        <div className={classes.downloadInstruction}>
          Copy this link and send it to the receiver or click below to generate
          QR code
        </div>
        ...
        {showQR && (
          ...
          >
            <Paper
              onClick={generateQR}
              className={classes.qrGenerateMenu}
              elevation={1}
            >
              <div
                style={{
        ...
        {!smallScreen && (
          <Paper elevation={0} className={classes.downloadPageLink}>
            {props.data.downloadPageLink}
          </Paper>
        )}
        {smallScreen && (
          <input
            className={classes.downloadPageLink}
            disabled
            value={props.data.downloadPageLink}
          ></input>
        )}

        <Button
          onClick={() => props.setGoBack(true)}
          color="primary"
          variant="contained"
      ...
      >
        <DialogTitle
          style={{ fontWeight: "bold", color: "#0c65c1", fontSize: 16 }}
        >
          {"Confirm Go Back?"}
        </DialogTitle>
        <DialogContent>
          <DialogContentText className={classes.dialogText}>
            Clicking 'Yes' will delete the data of the files you've uploaded.
            The action is irreversible. Only do so if you are done sharing files
            with the receiver.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
